...
UpdatePage::UpdatePage(QWidget *parent, JSONFUNC *json,ShareData *sharedata) : QWidget(parent)
{
    ...
    connect(this,SIGNAL(appUpdateOk(QString, QString, QString, QString)),this,SLOT(sendUpdateSuccess(QString)));
    createUpdateWindow();
}
...
void UpdatePage::onGetupFinished(UPDATESTRUCTMAP updateMap)
{
    ...
    for(itor = updateMap.begin(); itor != updateMap.end(); itor++)
    {
        ...
        QString appName = itor.value().proName;      // 注意：显示名——>包名
...
void UpdatePage::onAppUpdateFinished()
{
    for(int i = 0; i < updateTable->rowCount(); i++)
    {
        ...
        {
            qDebug() << "remove row :" << i;
            emit appUpdateOk(appWidget->getAppName() ,appWidget->getHeadUrl(), appWidget->getAppVer(), appWidget->getExeFile());
            if(!appWidget->getFuncButton()->isEnabled())
...
{
    emit sigUpdateSuccess(appName,true,2);
}

void UpdatePage::sendUpdateFailure(QString appName)
{
    emit sigUpdateSuccess(appName,false,2);
}
